...
class TestCollector:
  ...
  def check_default_collector_set(ctx):
    assert {'performance', 'conversion_action', 'ad_disapprovals',
            'mapping'} == {c.name for c in ctx.collector_set}
  ...
  def search_subregistry_collectors(ctx):
    assert {
        'keywords', 'search_terms', 'keyword_quality_score', 'click_share',
        'mapping'
  ...
  def performance_search_and_default_mapping_collectors(ctx):
    assert {'performance', 'mapping',
            'keywords'} == {c.name for c in ctx.collector_set}
  ...
  @behave.then('performance, mapping, keywords, ad_disapprovals, '
               'conversion_action collectors are returned')
  def collector_from_default_registry_and_keywords(ctx):
    assert {
        'performance', 'conversion_action', 'ad_disapprovals', 'mapping',
        'keywords'
    } == {c.name for c in ctx.collector_set}
