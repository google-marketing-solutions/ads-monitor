...
def simple_target():
  return Target(
      name='simple', metrics='impressions', level=TargetLevel.AD_GROUP)
...
def simple_target_at_mcc_level():
  return Target(
      name='simple_mcc_level', metrics='impressions', level=TargetLevel.MCC)
...
def complex_target():
  return Target(
      ...
      dimensions=[
          ...
          Field(name='search_terms_view.search_term', alias='search_term')
      ])
